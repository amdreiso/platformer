function draw_debug_gui(){
	...
	draw_text_outline(0, 2 * height, $"px {p.x} py {p.y} | hsp {pdir.x} vsp {pdir.y} | tx {t.x} ty {t.y}", scale, scale, 0, 1, font, back, fore);
	draw_text_outline(0, 3 * height, $"cx {c.x} cy {c.y}", scale, scale, 0, 1, font, back, fore);
