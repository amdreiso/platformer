function draw_raycast(obj, light_x, light_y, raycastCount = Settings.graphics.raycastCount, max_distance = 50, width = 1){
	...
	for (var i = 0; i < rays; i++) {
		...
		for (var j = 0; j < dist; j += step) {
			...
			var hit = collision_point(px, py, obj, false, true);
			if (hit != noone) {
				hit_x = floor(px);
				hit_y = floor(py);
				break;
